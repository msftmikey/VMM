...
void tlb_add(int index, page_node entry) {
	if (REPLACE_POLICY == LRU) {
		...
		int j;
		for (; i < TLB_SIZE; ++ i) {
			if (tlb[i].frame_num == entry.frame_num) {
				j = i;
				goto skip_lbl;
			}
		}
...
void tlb_remove(int index) { 
	...
	tlb[index].frame_num = (size_t) - 1;
	tlb[index].is_present = false;
}

void fifo_replace_page(size_t&);
void lru_replace_page(size_t&);
...
void page_fault(
	size_t& frame, size_t& page, size_t& frames_used, size_t& pg_faults,
...
void check_address_value(
	size_t logic_add, size_t page, size_t offset, size_t physical_add,
	size_t& prev_frame, size_t frame, int val, int value, size_t o
) { 
	printf(
		"log: %5lu 0x%04zu (pg:%3lu, off:%3lu)-->phy: %5lu (frm: %3lu) (prv: %3lu)--> val: %4d == value: %4d -- %s",
